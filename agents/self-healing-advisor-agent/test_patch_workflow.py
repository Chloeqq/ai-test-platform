...
def test_patch_preview_rejects_stable_smoke_case(tmp_path: Path):
    ...
    assert plan["boundary"]["allowed"] is False
...
def test_apply_and_rollback_patch_workflow(tmp_path: Path):
    ...
    assert plan["boundary"]["allowed"] is True
    assert plan["boundary"]["advice_type"] == "locator_update"
...
def test_self_healing_orchestrator_keeps_patch_when_rerun_succeeds(tmp_path: Path):
    ...
    assert result["plan"]["boundary"]["allowed"] is True
...
def test_patch_preview_rejects_assertion_update_outside_boundary(tmp_path: Path):
    ...
    assert plan["allowed_to_apply"] is False
    assert "outside deterministic auto-healing boundary" in plan["reason"]
    assert plan["boundary"]["allowed"] is False
    assert "assertion_update" in plan["boundary"]["preview_only_advice_types"]
