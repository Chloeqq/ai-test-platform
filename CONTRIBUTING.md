...
说明：当前 CI 在 `pull_request/push` 默认执行 `make static-baseline`（当前范围：`ruff` + scoped `mypy` + `pytest -q agents` + `runners` 工具链稳定测试集 + `ai-orchestrator` OpenAPI 合约测试），本地提前执行可避免命令口径不一致导致的回归。
