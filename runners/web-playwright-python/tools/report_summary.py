...
except Exception:  # pragma: no cover - keep report summary resilient
...
def _load_records_from_manifest(manifest_path: Path) -> list[dict[str, str]]:
    ...
    if normalize_evidence_manifest_v1 is not None and manifest_raw:
        try:
            manifest, _warnings = normalize_evidence_manifest_v1(manifest_raw)
        except Exception:
            manifest = manifest_raw
    if not manifest:
        return []
    if manifest_path.name == "evidence_manifest.json":
        ...
        if str(manifest.get("schema_version", "")).strip() not in valid_versions and str(manifest.get("version", "")).strip() not in valid_versions:
            return []
