...
from runners.api_python.gate.test_reliability_gate import check_reliability
...
def execute_api_test(
    ...
    *,
    ...
    api_results = _evaluate_assertions(response_status, response_body, intent.assertions)
    ...
    if intent.db_assertions and db_url:
        db_results, db_evidence = _execute_db_assertions(intent.db_assertions, sut_db_url=db_url)
        ...
        result.assertions.append(AssertionResult(
            type="db", expected="DB assertions defined",
            actual="ATP_SUT_DB_URL not set", passed=False,
        ))
        result.evidence = evidence

    all_passed = all(a.passed for a in result.assertions)
    result.status = "passed" if all_passed else "failed"
    result.failure_layer = None if all_passed else "business_rule"
    return result
