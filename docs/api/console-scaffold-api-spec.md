...
本文档覆盖的是 `Scaffold Console` 依赖的资产脚手架接口，不是 URL-first workbench 的生成、执行、review 或 gate API 总表。
...
- 这组接口主要面向“模板驱动的测试资产 scaffold”。
- 它们不等同于 URL-first 主链里的 `/api/workbench/auto-run`、`/api/workbench/runs/*`、`/api/workbench/reviews`、`/api/workbench/execution-gate/*`。
