...
| # | 问题 | 影响 | 修复方案 |
|---|------|------|---------|
| P0-5 | `payloadPoint()` 总生成 candidate_step | 编辑→保存闭环断裂 | 不生成 candidate_step, 发送原始 steps 文本让后端重新编译 |
| P0-6 | `upsert_test_point_asset` 直接替换 points | 不经过 structurer 重新编译 | incoming_points 时也调用 build_point() 重新编译 |
| P0-7 | 前端 `LOGIN_INVOLVED_ELEMENT_ALIASES` | 硬编码 login 页面 | 从 page_elements API 动态加载 |
| P0-8 | 前端 `page !== "login"` | 硬编码页面判断 | 删除条件, 所有页面统一走 API |
