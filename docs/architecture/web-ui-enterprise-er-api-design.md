...
- 这是历史阶段的数据库与 API 快照，不是当前 workbench runtime 的完整真相。
- 目前生成页里的 `review_state`、`execution_gate`、确认点审计和页面分析回显，更多是文件化/运行态逻辑，不一定已经沉到这份 ER 里。
- 因此，本文更适合作为“平台化数据底座蓝图”，而不是当前可直接执行的唯一事实源。
