...
def test_openapi_spec_declares_asset_management_request_schemas():
    ...
    assert "ExecutionStepSummary" in schemas
    assert "ExecutionEvidenceIndex" in schemas
...
def test_openapi_spec_declares_report_fields_on_orchestration_result():
    ...
    assert result_schema["properties"]["test_points"]["$ref"] == "#/components/schemas/TestPointPlan"
    assert result_schema["properties"]["execution_record"]["$ref"] == "#/components/schemas/ExecutionRecord"
    assert response_schema["properties"]["execution_record"]["$ref"] == "#/components/schemas/ExecutionRecord"
...
def test_openapi_spec_declares_pytest_and_failure_analysis_fields():
    ...
    assert report_schema["properties"]["execution_record"]["$ref"] == "#/components/schemas/ExecutionRecord"
    ...
    assert "analysis_files" in spec["components"]["schemas"]["ExecutionEvidenceSummary"]["properties"]
    assert "suggestion_files" in spec["components"]["schemas"]["ExecutionEvidenceSummary"]["properties"]
    assert "execution_record_files" in spec["components"]["schemas"]["ExecutionEvidenceSummary"]["properties"]
    assert "self_healing_result_files" in spec["components"]["schemas"]["ExecutionEvidenceSummary"]["properties"]
