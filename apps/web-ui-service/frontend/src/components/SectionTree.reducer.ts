/** SectionTree reducer：纯状态管理（类型 + reducer + helpers）。独立文件,便于单测。 */

import { type SectionNode as ApiSectionNode } from "../api/workbench";
...
export function adaptNodes(apiNodes: ApiSectionNode[]): SectionNode[] {
...
export function sectionTreeReducer(
  ...
  switch (action.type) {
    ...
    case "CHECK_ALL": {
      ...
      return { ...state, checkedIds: new Set(allIds), indeterminateIds: new Set() };
    }

    case "UNCHECK_ALL": {
      return { ...state, checkedIds: new Set(), indeterminateIds: new Set() };
    }
