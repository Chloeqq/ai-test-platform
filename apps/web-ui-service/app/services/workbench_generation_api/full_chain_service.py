...
class FullChainPipelineState:
    ...
    test_point_sync: dict[str, Any] = field(default_factory=dict)
...
class FullChainPipeline:
    ...
    def stage_sync_test_points(self, state: FullChainPipelineState) -> None:
        preview_item = state.preview_payload.get("item") if isinstance(state.preview_payload, dict) else {}
        preview_item = preview_item if isinstance(preview_item, dict) else {}
        requirement_spec = preview_item.get("requirement_spec") if isinstance(preview_item.get("requirement_spec"), dict) else {}
        state.test_point_sync = test_point_service.sync_requirement_test_points(
            self._repository.db,
            project_code=_normalize_chain_text(state.payload.project) or "atp",
            page_code=_normalize_chain_text(preview_item.get("page")) or state.normalized_page or "common",
    ...
    def stage_generate_cases(self, state: FullChainPipelineState) -> None:
        ...
        generate_payload = GenerateCasePayload(
            project=_normalize_chain_text(state.payload.project) or "atp",
    ...
    def stage_build_response(self, state: FullChainPipelineState) -> dict[str, Any]:
        return {
            "summary": {
                ...
                "persisted_test_point_count": int(state.test_point_sync.get("total_count", 0) or 0),
                "inserted_test_point_count": int(state.test_point_sync.get("inserted_count", 0) or 0),
                "updated_test_point_count": int(state.test_point_sync.get("updated_count", 0) or 0),
            },
            "stages": {
                "test_point_matrix": state.preview_payload.get("item", {}) if isinstance(state.preview_payload, dict) else {},
                "test_point_store": state.test_point_sync,
                "coverage_matrix": state.coverage_matrix,
    ...
    def run(self, payload: Any) -> dict[str, Any]:
        ...
        self.stage_sync_test_points(state)
