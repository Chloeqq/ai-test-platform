...
ReadAllureEnvironment = Callable[[], list[dict[str, Any]]]
ReadAllureExecutors = Callable[[], list[dict[str, Any]]]
EnsureAllureSnapshot = Callable[..., str]
...
def inject_allure_branding(html_content: str) -> str:
    ...
    if "</head>" not in source:
        return f"{source}\n{custom_style}"
    return source.replace("</head>", f"{custom_style}\n</head>", 1)
...
def build_report_allure(
    *,
    ...
    return {
        ...
        "snapshot_slug": identity["snapshot_slug"],
        "summary": summary,
        "environment": environment,
        "executors": executors,
        "report_source": "run_scoped",
        "results_dir": str(current_results_dir),
    }
...
def build_allure_report_identity(*, summary: dict[str, Any], cache_version: int) -> dict[str, str]:
    ...
    if raw_report_name and raw_report_name != "Allure Report":
        report_name = raw_report_name
    else:
        report_name = f"AI 自动化测试执行报告 ({display_version})"
    return {
