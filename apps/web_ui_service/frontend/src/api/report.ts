...
export async function getExecutionReportDetail(executionId: string, runId?: string): Promise<ExecutionReportDetailResponse> {
  const normalized = String(executionId || "").trim();
  const query = runId ? `?run_id=${encodeURIComponent(runId)}` : "";
  return getJson<ExecutionReportDetailResponse>(`/api/report/executions/${encodeURIComponent(normalized)}${query}`);
}
